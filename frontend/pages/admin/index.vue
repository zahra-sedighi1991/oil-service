...
<template>
  <div class="list-page">
    <div class="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
      <div v-for="item in [{ label: 'همه فروشگاه‌ها', value: counts.all }, { label: 'فعال', value: counts.active }, { label: 'در انتظار', value: counts.pending }, { label: 'تعلیق‌شده', value: counts.suspended }]" :key="item.label" class="card p-4"><strong class="text-2xl font-800">{{ number(item.value) }}</strong><span class="mt-1 block text-xs text-ink/45">{{ item.label }}</span></div>
    </div>
