...
<template>
  <div class="list-page">
    ...
    <section class="list-panel">
      <div v-if="suggestions?.length" class="scroll-container list-scroll card-stack">
        <div v-for="item in suggestions" :key="item.id" class="card flex flex-col gap-3 p-5 sm:flex-row sm:items-center">
      ...
      <AppEmptyState v-else class="card" icon="i-lucide-lightbulb" title="پیشنهادی ثبت نشده" description="محصولات و خدمات خارج از کاتالوگ هنگام نهایی‌سازی سفارش، با نام واقعی‌شان به‌صورت خودکار اینجا قرار می‌گیرند." />
    </section>
  </div>
</template>
