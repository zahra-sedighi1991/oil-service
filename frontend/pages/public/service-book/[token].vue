...
<template>
  <main class="min-h-screen px-4 py-6 sm:py-10">
    <div v-if="error" class="card mx-auto max-w-lg p-8 text-center">
      ...
      <p class="mb-0 mt-2 text-sm leading-6 text-muted">برای دریافت لینک جدید با تعویض روغنی تماس بگیرید.</p>
    </div>
    <div v-else-if="book" class="mx-auto max-w-3xl">
      <header class="card overflow-hidden">
        <div class="bg-ink p-6 text-white sm:p-8">
          ...
          <div class="mt-3 flex flex-wrap gap-2 text-sm text-white/70"><span>{{ book.vehicle.plate }}</span><span>•</span><span>{{ number(book.vehicle.lastOdometer) }} کیلومتر</span></div>
        </div>
        <div v-if="book.nextDue" class="grid gap-3 p-5 sm:grid-cols-2">
          <div v-if="book.nextDue.dueOdometer" class="rounded-xl bg-amber-50 p-4"><span class="block text-xs text-amber-700">سررسید کیلومتری بعدی</span><strong class="mt-1 block">{{ number(book.nextDue.dueOdometer) }} کیلومتر</strong><small class="text-muted">{{ book.nextDue.dueOdometerItem }}</small></div>
          <div v-if="book.nextDue.dueDate" class="rounded-xl bg-brand-50 p-4"><span class="block text-xs text-brand-700">سررسید زمانی بعدی</span><strong class="mt-1 block">{{ date(book.nextDue.dueDate) }}</strong><small class="text-muted">{{ book.nextDue.dueDateItem }}</small></div>
        </div>
      </header>
      ...
      <section class="mt-5">
        ...
        <div v-if="book.services?.length" class="space-y-4">
          <article v-for="service in book.services" :key="service.id" class="card p-5">
            <div class="mb-5 flex items-start justify-between gap-3"><div><strong class="block">{{ date(service.serviceDate) }}</strong><span class="mt-1 block text-xs text-muted">{{ number(service.odometer) }} کیلومتر</span></div><span class="badge bg-brand-50 text-brand-700">{{ service.invoice?.invoiceNo }}</span></div>
            ...
            <div v-if="service.invoice" class="mt-4 flex items-center justify-between border-t border-dashed border-black/8 pt-4 text-sm"><span class="text-muted">مبلغ فاکتور</span><strong>{{ money(service.invoice.totalAmount, service.invoice.currency) }}</strong></div>
          </article>
        </div>
      ...
      <footer class="mt-7 rounded-2xl bg-ink p-5 text-center text-white">
        ...
        <p class="mb-0 mt-2 text-xs text-white/70">{{ [book.shop.city, book.shop.address].filter(Boolean).join('، ') }}</p>
