...
<template>
  <AppModal :open="open" :title="title" :description="description" @close="emit('close')">
    <form class="space-y-4" @submit.prevent="submit">
      ...
      <div>
        ...
        <input v-model="form.name" class="field" required placeholder="مثلاً روغن موتور بهران سوپر پیشتاز">
      </div>
      <div class="grid grid-cols-2 gap-3">
        ...
        <div><label class="label">حجم <span class="font-400 text-ink/40">(اختیاری)</span></label><input v-model.number="form.packageVolume" type="number" min="0" step="0.1" class="field text-left" dir="ltr" placeholder="مثلاً 4"></div>
      </div>
        ...
        <div v-if="!appliesToAllVehicles" class="mt-3">
          ...
          <div class="scroll-container mt-2 max-h-52 space-y-1 overflow-y-auto">
            <label v-for="model in filteredVehicleModels" :key="model.id" class="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 hover:bg-black/[.03]">
              <input type="checkbox" :checked="form.vehicleModelIds.includes(model.id)" class="accent-brand-600" @change="toggleVehicle(model.id)">
          ...
          <p v-if="!form.vehicleModelIds.length" class="mb-0 mt-1 text-xs text-danger">حداقل یک مدل خودرو انتخاب کنید.</p>
        </div>
      </div>
      <button class="btn-primary w-full" :disabled="saving || (!appliesToAllVehicles && !form.vehicleModelIds.length)">
        <span v-if="saving" class="i-lucide-loader-circle h-4 w-4 animate-spin" />
